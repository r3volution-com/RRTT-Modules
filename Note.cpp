...
Note::Note(Texture *nTex, Rect<float> *nRect, Texture *bTex, Rect<float> *bRect, Font *f) {
    ...
    text = new Text(std::string(), new Coordinate(0, 0), f, true);
    taken = false;
}
...
void Note::setBackgroundPosition(Coordinate *pos){
    ...
    text->setPosition(new Coordinate(pos->x+(backgroundSprite->getActualSpriteRect()->w/2), pos->y+(backgroundSprite->getActualSpriteRect()->h/2)));
}
